<template>
    <el-container class="god_container">
      ...
      <el-header class="god_header">
         <div>
            ...
            <span class="title">尊敬的领导 你好</span>
  ...
  methods:{
    ...
    //  }
      ...
      const {data:res} = await this.$http.get('events/getEventsAllInfoPage',{
    ...
    async search() {
      const {data:res} = await this.$http.get('school/getSchoolPage',{
        params: {
          schoolName:this.searchItem
        }
      })
      console.log(res.data.records[0])
      this.queryInfo.schoolId = res.data.records[0].schoolId
      this.getDataList()
    },
    editItemByid(item) {
        console.log(item)
    ...
    const confirmResult = await this.$confirm(
          ...
          //确认
       const {data:res} = await this.$http.get('events/schoolConfimEvents', {
        params: {
