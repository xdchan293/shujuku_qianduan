  ...
  <div class="student_main"> 
    <el-card>
     ...
     <el-table :data="datalist" border stripe>
            ...
            <el-table-column  label="辅导员" prop="teacherName" ></el-table-column>
            <el-table-column  label="系" prop="deptName" ></el-table-column>
    ...
    <el-dialog
      ...
      <el-form 
      :model="addForm">
      <el-form-item label="专业名" class="input_item">
       <el-input 
        style="width:220px"
      ...
      <el-form-item label="辅导员" class="input_item">
       <el-input 
        style="width:220px"
      ...
      <el-form-item label="学院" class="input_item">
       <el-input 
        style="width:220px"
      ...
      <!-- <el-form-item label="学院id" class="input_item">
       <el-input 
        style="width:220px"
...
methods: {
  ...
  //  }
    ...
    const res = await this.$http.get('major/getMajorPage',{
  ...
  async addItem() {
   //todo 发送添加请求
   //先请求到辅导员和系的内容
   const {data:res1} = await this.$http.get('teacher/getTeacherPage',{
    params:{
      teacherName:this.addForm.teacherName
    }
   })

   const {data:res2} = await this.$http.get('dept/getDeptPage',{
    params:{
  ...
  //  console.log(res2.data.records[0])

   const {data:res3} = await this.$http.post('major/createMajor',{
    majorName:this.addForm.majorName,
    teacherName:this.addForm.teacherName,
    deptName:this.addForm.deptName,
    schoolName:this.addForm.schoolName,
    teacherId:res1.data.records[0].teacherId,
    deptId:res2.data.records[0].deptId,
    schoolId:res2.data.records[0].schoolId
 ...
 async delItemByid(item) {
    const confirmResult = await this.$confirm(
          ...
          //todo
          //确认的话就删除
   const {data:res} = await this.$http.get('major/deleteMajor', {
        params: {
