  ...
  <div class="student_main"> 
    <el-card>
     ...
     <el-table :data="datalist" border stripe>
            ...
            <el-table-column  label="校区" prop="schoolLocation" ></el-table-column>
            <el-table-column  label="学院id" prop="schoolId" ></el-table-column>
            <el-table-column  label="校区id" prop="schoolLocationId" ></el-table-column>
    ...
    <el-dialog
      ...
      <el-form-item label="校区" class="input_item">
       <el-input 
        style="width:220px"
        v-model="addForm.schoolLocation"
        autocomplete="off"
        clearable
        ></el-input>
      </el-form-item>

      <el-form-item label="校区id" class="input_item">
       <el-input 
        style="width:220px"
        v-model="addForm.schoolLocationId"
        autocomplete="off"
        clearable
        ></el-input>
      </el-form-item>

      <span>（1本部 2南校 3华文 4珠海 5深圳）</span>
...
methods: {
  async getDataList() {
    ...
    const res = await this.$http.get('school/getSchoolPage',{
  ...
  async addItem() {
   //todo 发送添加请求
   const {data:res} = await this.$http.post('/school/createSchool',this.addForm)
   console.log(res)
   if(res.code!==200)
   {
 ...
 async delItemByid(item) {
    const confirmResult = await this.$confirm(
      ...
      const {data:res} = await this.$http.get('/school/deleteSchool', {
        params: {
